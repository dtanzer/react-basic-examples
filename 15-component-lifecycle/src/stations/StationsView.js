...
export class StationsView extends React.Component {
	...
	renderSubwayLine(lineData, lineId) {
		...
		const renderedLinks = links.map((linkData) => {
			return <Link container={this.cells} from={ linkData.current } to={ linkData.next } />
		});
		return renderedLinks;
	}

	renderStation(station) {
		return <Station container={this.cells} station={station} onChangedPosition={ this.props.onChangedPosition } />;
	}

	render() {
		const allStations = this.props.stations.u1.concat(this.props.stations.u2);
		//const allStations = allStations.reduce((prev, cur) => { return prev.concat(cur) }).toSet();
